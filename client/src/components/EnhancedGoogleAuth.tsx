...
const EnhancedGoogleAuth: React.FC<EnhancedGoogleAuthProps> = ({
    ...
    const [googleServicesError, setGoogleServicesError] = useState<string | null>(null);
    ...
    const handleGoogleCredentialResponse = async (response: GoogleCredentialResponse) => {
        try {
            ...
            const message = error instanceof Error ? error.message : 'Failed to process Google authentication';
            onError?.(message);
        }
    };
    ...
    const handleRetry = async () => {
        if (retryCount >= 3) {
            onError?.('Maximum retry attempts reached. Please refresh the page and try again.');
            return;
        }

        setRetryCount(prev => prev + 1);
        console.log(`🔄 Retrying Google authentication (attempt ${retryCount + 1}/3)...`);
    ...
    const isLoading = isGoogleAuthLoading || (mode === 'client' && !isGoogleServicesLoaded && !googleServicesError);
